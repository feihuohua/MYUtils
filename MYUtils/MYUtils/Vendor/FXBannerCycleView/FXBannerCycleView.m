...
#import "FXBannerCycleFlowLayout.h"
#import "UIView+Extension.h"
#import "UIColor+Extension.h"

@interface FXBannerCycleView ()<UICollectionViewDelegate,UICollectionViewDataSource>

@property (nonatomic, assign) FXBannerCycleViewType type;
...
@implementation FXBannerCycleView
...
- (void)layoutSubviews
{
    ...
    switch (self.pageControlPosition) {
        case FXBannerCyclePageControlPositionCenter: {
        ...
        case FXBannerCyclePageControlPositionLeft: {
            self.pageControl.left = 0 + self.pageControlOffset.x;
            self.pageControl.bottom = self.collectionView.bounds.size.height - 15.0f + self.pageControlOffset.y;
            break;
        }
        case FXBannerCyclePageControlPositionRight: {
...
{
    if (self.type == FXBannerCycleViewTypeDefault) {
...
- (void)reloadData
{
    ...
    if (self.itemCount > 1 && self.type == FXBannerCycleViewTypePage && self.timeInterval > 0) {
        [self startTimer];
    } else {
        [self stopTimer];
    }
...
{
    if (self.type == FXBannerCycleViewTypePage) {
...
- (UICollectionView *)collectionView
{
    if (_collectionView == nil) {
        ...
        switch (self.type) {
            case FXBannerCycleViewTypeDefault:
            {
                FXBannerCycleFlowLayout *flowLayout = [[FXBannerCycleFlowLayout alloc] init];
            ...
            case FXBannerCycleViewTypePage:
            {
