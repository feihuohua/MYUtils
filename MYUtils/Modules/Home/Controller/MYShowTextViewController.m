...
- (void)viewDidLoad {
    ...
    for (NSInteger i = 0; i < [contentArr count]; i++) {
        ...
        MYShowTextCellModel *cellModel = [[MYShowTextCellModel alloc] initWithContent:contentArr[i] contentLines:TextInitContentLines isOpen:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!cell) {
        cell = [[MYShowTextViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier];
        __weak typeof(self) weakSelf = self;
        [cell setOpenContentBlock:^(MYShowTextCellModel *cellModel){
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (cellModel.isOpen) {
                cellModel.contentLines = TextContentNoLimitLines;  //0,不限制行数
            } else {
                cellModel.contentLines = TextInitContentLines;     //3,3行
            }
