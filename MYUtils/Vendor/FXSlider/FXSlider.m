...
@property (nonatomic, assign) CGPoint startSliderPosition;
@property (nonatomic, assign) CGPoint startTouchPosition;
@property (nonatomic, assign) CGFloat diff;
@property (nonatomic, assign) CGFloat maxRadius;
@property (nonatomic, assign) BOOL animateLayouts;
@property (nonatomic, strong) CAShapeLayer *trackLayer;
@property (nonatomic, strong) CAShapeLayer *sliderCircleLayer;
@property (nonatomic, strong) NSMutableArray <CAShapeLayer *> *trackCirclesArray;
@property (nonatomic, strong) NSMutableArray <CATextLayer *> *trackLabelsArray;
...
- (void)layoutLayersAnimated:(BOOL)animated
{
    ...
    CGFloat sliderHeight = fmaxf(_maxRadius, self.trackHeight / 2.f) * 2.f;
    ...
    _contentSize = CGSizeMake(fmaxf(44.f, self.bounds.size.width), fmaxf(44.f, sliderHeightWithLabels));
    if (!CGSizeEqualToSize(self.bounds.size, _contentSize)) {
        if (self.constraints.count) {
            [self invalidateIntrinsicContentSize];
        } else {
            CGRect newFrame = self.frame;
            newFrame.size = _contentSize;
            self.frame = newFrame;
        }
    }
    ...
    if (self.labelOrientation == FXStepSliderTextOrientationUp && self.labels.count) {
        contentFrameY += labelHeight + self.labelOffset;
    }
    
    CGRect contentFrame = CGRectMake(_maxRadius,
                                     contentFrameY,
                                     self.bounds.size.width - 2 * _maxRadius,
                                     sliderHeight);
...
- (void)layoutSubviews
{
    ...
    [self layoutLayersAnimated:_animateLayouts];
    _animateLayouts = NO;
}
...
- (void)updateDiff
{
    _diff = sqrtf(fmaxf(0.f, powf(self.trackCircleRadius, 2.f) - pow(self.trackHeight / 2.f, 2.f)));
}

- (void)updateMaxRadius
{
    _maxRadius = fmaxf(self.trackCircleRadius, self.sliderCircleRadius);
}
...
{
    return _sliderCircleLayer.position.x - _maxRadius;
}

- (CGFloat)trackCirclePosition:(CAShapeLayer *)trackCircle
{
    return trackCircle.position.x - _maxRadius;
}
...
{
    return self.sliderPosition + _diff >= [self trackCirclePosition:trackCircle] ? self.tintColor.CGColor : self.trackColor.CGColor;
}
...
{
    _startTouchPosition = [touch locationInView:self];
    _startSliderPosition = _sliderCircleLayer.position;
    
    if (CGRectContainsPoint(_sliderCircleLayer.frame, _startTouchPosition)) {
        return YES;
        ...
        for (NSUInteger i = 0; i < _trackCirclesArray.count; i++) {
            ...
            if (CGRectContainsPoint(frameToCheck, _startTouchPosition)) {
...
{
    CGFloat position = _startSliderPosition.x - (_startTouchPosition.x - [touch locationInView:self].x);
    CGFloat limitedPosition = fminf(fmaxf(_maxRadius, position), self.bounds.size.width - _maxRadius);
    
    withoutCAAnimation(^{
        _sliderCircleLayer.position = CGPointMake(limitedPosition, _sliderCircleLayer.position.y);
        _trackLayer.path = [self fillingPath];
        
        NSUInteger index = (self.sliderPosition + _diff) / (_trackLayer.bounds.size.width / (self.maxCount - 1));
        if (_index != index) {
    ...
    if (index >= _trackLabelsArray.count) {
        ...
        if (self.adjustLabel) {
            if (index == 0) {
                ...
                size.width /= 2.f;
                anchorPoint.x = _maxRadius / size.width;
            } else if (index == self.labels.count - 1) {
                alignmentMode = kCAAlignmentRight;
                size.width /= 2.f;
                anchorPoint.x = 1.f - _maxRadius / size.width;
            }
        }
...
GENERATE_SETTER(labelOrientation, FXStepSliderTextOrientation, setLabelOrientation, );
