...
@implementation MYLaunchAdvertCache
...
+ (BOOL)saveImageData:(NSData *)data imageURL:(NSURL *)url {
    ...
    if (data) {
        ...
        if (!result) {
            MYLog(@"cache file error for URL: %@", url);
        }
        return result;
    }
    return NO;
...
+ (NSString *)launchAdCachePath {
    NSString *path = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/MYLaunchAdCache"];
...
+ (NSString *)videoPathWithFileName:(NSString *)videoFileName {
    if ([NSString isEmpty:videoFileName]) {
        return nil;
    }
...
+ (void)async_saveImageUrl:(NSString *)url {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[NSUserDefaults standardUserDefaults] setObject:url forKey:MYCacheImageUrlStringKey];
        [[NSUserDefaults standardUserDefaults] synchronize];
    });
}

+ (NSString *)getCacheImageUrl {
    return [[NSUserDefaults standardUserDefaults] objectForKey:MYCacheImageUrlStringKey];
}
...
+ (void)async_saveVideoUrl:(NSString *)url {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[NSUserDefaults standardUserDefaults] setObject:url forKey:MYCacheVideoUrlStringKey];
        [[NSUserDefaults standardUserDefaults] synchronize];
    });
}

+ (NSString *)getCacheVideoUrl {
    return [[NSUserDefaults standardUserDefaults] objectForKey:MYCacheVideoUrlStringKey];
}
...
+ (void)clearDiskCacheExceptImageUrlArray:(NSArray<NSURL *> *)exceptImageUrlArray {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [allFilePaths enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (![exceptImagePaths containsObject:obj] && !MYISVideoTypeWithPath(obj)) {
                [[NSFileManager defaultManager] removeItemAtPath:obj error:nil];
            }
        }];
        MYLog(@"allFilePath = %@",allFilePaths);
    });
}
...
+ (void)clearDiskCacheExceptVideoUrlArray:(NSArray<NSURL *> *)exceptVideoUrlArray {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [allFilePaths enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (![exceptVideoPaths containsObject:obj] && MYISVideoTypeWithPath(obj)) {
                [[NSFileManager defaultManager] removeItemAtPath:obj error:nil];
            }
        }];
        MYLog(@"allFilePath = %@",allFilePaths);
...
+ (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        MYLog(@"create cache directory failed, error = %@", error);
    } else {
        [self addDoNotBackupAttribute:path];
    }
    MYLog(@"XHLaunchAdCachePath = %@",path);
}
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        MYLog(@"error to set do not backup attribute, error = %@", error);
    }
}

+ (NSString *)videoNameWithURL:(NSURL *)url {
    return [url.absoluteString.md5String stringByAppendingString:@".mp4"];
}
